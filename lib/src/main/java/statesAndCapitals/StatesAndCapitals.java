...
public class StatesAndCapitals
{
    ...
    public void testStatesAndCapitals() throws IOException
    {
        ...
        List<StateInfo> firstFiveStates = states.stream().limit(5)
        ...
        StateInfo cardinalState =  states.stream().filter(x -> x.getStateBird().equals("cardinal")).findFirst().orElseThrow();
    ...
    testResults.put("I1", StatesAndCapitalsCheck.int1( cardinalState));
        ...
        Boolean isAnyStateLessThan0Elevation = states.stream().anyMatch(x-> x.getLowestElevationInFeet()<0);
        ...
        Boolean isAnyStateGreaterThan21000Elevation = states.stream().anyMatch(x -> x.getHighestElevationInFeet()>21000);
        ...
        Boolean doAllStatesHaveAnAnthem = states.stream().allMatch(x-> x.getStateAnthem().isEmpty()!=false);
        ...
        Boolean doNoStatesHaveAOneWordMotto = states.stream().noneMatch(x -> x.getStateMotto().split("",1).length==1);
        ...
        Map<String, Long> numberOfStatesByTimeZone = states.stream().collect(groupingBy(y -> y.getTimeZones().toString(),counting()));
        ...
        Map<String, Long> numberOfStateCapitalsByTimeZone = states.stream().collect(groupingBy(y-> y.getCapital().getTimeZone(),counting()));
        ...
        List<String> stateTreesSortedAscending =  states.stream().map(x -> x.getStateTree()).sorted().collect(toList());
        ...
        String allStateNamesSemicolonDelimited = states.stream().map(x -> x.getStateName()).collect(Collectors.joining("; "));
//System.out.println(allStateNamesSemicolonDelimited);
        ...
        List<String> allDistinctStateBirds = states.stream().map(x -> x.getStateBird()).distinct().collect(toList());
        ...
        List<String> allDistinctStateBirdsMinusMockingbirds = states.stream().filter(x-> !x.getStateBird().contains("mockingbird")).map(x -> x.getStateBird()).distinct().collect(toList());
        ...
        Long numberOfDistinctStateBirds =  states.stream().map(x -> x.getStateBird()).distinct().collect(counting());
